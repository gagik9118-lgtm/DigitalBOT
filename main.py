...
def init_db():
    try:
        ...
        if not c.fetchone():
            c.execute("INSERT INTO users (user_id, username, full_name, status, registered_at) VALUES (?, ?, ?, ?, ?)",
                      (OWNER_ID, "opps911", "Владелец", "owner", datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
            conn.commit()
        conn.close()
...
def save_user(message):
    try:
        ...
        c.execute("INSERT INTO users (user_id, username, full_name, status, registered_at) VALUES (?, ?, ?, ?, ?)",
                  (user_id, username, full_name, status, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit()
        conn.close()
    except Exception as e:
...
Вы платите за разработку системы один раз, а экономите миллионы на фонде оплаты труда каждый год. <b>Нажмите кнопку ниже, чтобы обсудить автоматизацию вашего проекта</b> 👇
        ...
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(types.InlineKeyboardButton("🤝 Партнёрская программа", callback_data="partner_program"))
