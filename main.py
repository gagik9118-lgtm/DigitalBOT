...
def privacy_required(func):
    """Декоратор для проверки согласия с политикой — блокирует ВСЁ, включая /partner"""
    @wraps(func)
    def wrapper(message, *args, **kwargs):
        user_id = message.from_user.id
...
def privacy_required_callback(func):
    ...
    def wrapper(call, *args, **kwargs):
        ...
        # Пропускаем только accept_privacy
...
Вы платите за разработку системы один раз, а экономите миллионы на фонде оплаты труда каждый год. <b>Нажмите кнопку ниже, чтобы обсудить автоматизацию вашего проекта</b> 👇
        ...
        # ОТДЕЛЬНОЕ МЕНЮ УСЛУГ (ReplyKeyboard) — привязано к этому же сообщению
        # НО без текста "выберите услугу"
...
@check_banned
@privacy_required  # ← ПОЛИТИКА БЛОКИРУЕТ /partner
def partner_command(message):
    """Команда /partner — открывает ЛК партнёра"""
    user_id = message.from_user.id
...
@check_banned
@privacy_required  # ← ПОЛИТИКА БЛОКИРУЕТ ЛЮБЫЕ СООБЩЕНИЯ
...
Завершённых сделок за всё время: {closed}
        ...
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("🔙 В меню", callback_data="admin_back"))
...
def process_search_apps(message):
    try:
        ...
        if query.isdigit():
            c.execute("SELECT * FROM applications WHERE client_id = ? ORDER BY created_at DESC", (int(query),))
        else:
            c.execute("SELECT * FROM applications WHERE created_at LIKE ? OR service LIKE ? ORDER BY created_at DESC",
                      (f"%{query}%", f"%{query}%"))
        ...
        for app in apps[:5]:
            app_id, client_id, username, service, answers, status, created_at, referrer = app
            status_emoji = "🆕" if status == 'pending' else "✅" if status == 'closed' else "🚫"
            
            text = f"""
{status_emoji} Заявка #{app_id}
👤 @{username or 'Нет'} [ID: <code>{client_id}</code>]
📋 {service}
📅 {created_at[:16]}
